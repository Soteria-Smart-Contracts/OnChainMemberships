...
contract LiquidSubscription {
    ...
    function PurchaseSubscription(MembershipType _MembershipType) public payable returns(uint256 NewSubscriptionTokenID, bool success){
    ...
    function SplitSubscription(uint256 SubscriptionID, address Recipient, uint256 Weeks) public returns(uint256 NewSubscriptionTokenID){
        ...
        require(Subscriptions[SubscriptionID].TotalWeeksSubscribed >= Weeks && Subscriptions[SubscriptionID].SubscriptionExpiry > block.timestamp, "Insufficient weeks or subscription expired");
        ...
        uint256 TokenID = MembershipToken.Mint(Recipient);

        SubsctiptionInfo memory _SubscriptionInfo = SubsctiptionInfo({
            LastPurchaser: Recipient,
            LastPurchase: block.timestamp,
            MembershipType: Subscriptions[SubscriptionID].MembershipType,
            DiscountPercentage: Subscriptions[SubscriptionID].DiscountPercentage,
            SubscriptionExpiry: Subscriptions[SubscriptionID].SubscriptionExpiry,
            TotalWeeksSubscribed: Subscriptions[SubscriptionID].TotalWeeksSubscribed
        });
    ...
    function GetMembershipTypes() public view returns(MembershipType[] memory){
        ...
        for(uint256 i = 0; i < MembershipTypes.length; i++){
            _MembershipTypes[i] = MembershipTypes[i];
        }
...
contract StandardERC721 is ERC721Enumerable{
  ...
  uint256 public MaxUnMinted = 999999999999; //If this we're an array, this would be the equivalent of UnMinted.length


  constructor(string MembershipName, string MembershipTicker) ERC721(MembershipName, MembershipTicker){}
